...
.run(function($ionicPlatform,$rootScope,$cordovaSQLite) {
  ...
  // for opening a background db:
   ...
   $cordovaSQLite.execute(db, "CREATE TABLE IF NOT EXISTS ctg_colores (id integer primary key, nombre text, estatus integer, fecha_creacion text, fecha_modificacion text, usuario_creacion text, usuario_modificacion text)");

   // table for mechanismo
   $cordovaSQLite.execute(db, "CREATE TABLE IF NOT EXISTS ctg_mecanismos (id integer primary key, nombre text, estatus integer, fecha_creacion text, fecha_modificacion text, usuario_creacion text, usuario_modificacion text)");

   // table for ctg_tapiz
   $cordovaSQLite.execute(db, "CREATE TABLE IF NOT EXISTS ctg_tapiz (id integer primary key, nombre text, estatus integer, fecha_creacion text, fecha_modificacion text, usuario_creacion text, usuario_modificacion text)");

  // table for productos
   $cordovaSQLite.execute(db, "CREATE TABLE IF NOT EXISTS productos (id_producto integer primary key, nombre text, tapiz_id integer, mecanismo_id integer, masaje_id integer, color_id integer, ancho integer, alto integer, profundo integer, estatus integer, categoria_id integer, fecha_creacion text, fecha_modificacion text, usuario_creacion text, usuario_modificacion text)");

   // table for producto_tienda
   $cordovaSQLite.execute(db, "CREATE TABLE IF NOT EXISTS producto_tienda (id integer primary key, producto_id integer, tienda_id integer)");

   // table for  producto_fotos
   $cordovaSQLite.execute(db, "CREATE TABLE IF NOT EXISTS  producto_fotos (id_foto integer primary key, producto_id integer, path text, orden integer)");
   
   // table for  ctg_categorias
   $cordovaSQLite.execute(db, "CREATE TABLE IF NOT EXISTS ctg_categorias (id integer primary key, nombre text, estatus integer, fecha_creacion text, fecha_modificacion text, usuario_creacion text, usuario_modificacion text)");
  
   // table for  ctg_masaje
   $cordovaSQLite.execute(db, "CREATE TABLE IF NOT EXISTS ctg_masaje (id integer primary key, nombre text, estatus integer, fecha_creacion text, fecha_modificacion text, usuario_creacion text, usuario_modificacion text)");
  


   // 

  // $rootScope.execute = function() {
  //   $cordovaSQLite.execute(db, "CREATE TABLE IF NOT EXISTS test_table (id integer primary key, data text, data_num text)");
